...
export function PageHero({ eyebrow, title, subtitle, backgroundImage }: { eyebrow?: string; title: string; subtitle?: string; backgroundImage?: string }) {
  if (backgroundImage) {
    return (
      <section className="relative aspect-[2.35/1] min-h-[300px] bg-fixed sm:bg-top bg-center bg-cover" style={{ backgroundImage: `url(${backgroundImage})` }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 container-page pb-8 sm:pb-12 md:pb-16">
          <div className="max-w-3xl">
            ...
            <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-white leading-tight">{title}</h1>
            {subtitle && <p className="mt-3 sm:mt-4 text-sm sm:text-base md:text-lg text-white/80 max-w-xl leading-relaxed">{subtitle}</p>}
          </div>
        </div>
      </section>
    );
